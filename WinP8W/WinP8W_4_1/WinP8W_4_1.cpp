...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_COLOR_CYAN:
			if (selected != -1)
				sectors[selected].color = RGB(0, 255, 255);
			break;
		case ID_COLOR_MAGENTA:
			if (selected != -1)
				sectors[selected].color = RGB(255, 0, 255);
			break;
		case ID_COLOR_YELLOW:
			if (selected != -1)
				sectors[selected].color = RGB(255, 255, 0);
			break;
